...
resultsworldList = resultsworldText[
    resultsworldText.index('USA'):resultsworldText.index('Total:')]

# Formats the world totals and adds them to a string variable to save.
resultstotalList = resultsworldText[
    resultsworldText.index('World'):resultsworldText.index('All')]
...
# Adds headers for the upcoming country totals and adds them to the
# string variable.
...
resultsusList = [
    x.text.strip() if x.text.strip() != '' else '0' for x in resultsusFine]

# Adds headers to the upcoming state totals and adds them to the string
# variable.
...
pathDir = os.path.join(
    os.path.expanduser('~'), 'Documents', 'PythonPrograms', 'VirusData')
